  ...
  return (
    <div className="relative pt-10 pb-6 px-6 max-w-5xl mx-auto w-full flex flex-col items-center">
      ...
      <motion.h1 
        ...
        className="font-serif italic text-4xl sm:text-6xl md:text-7xl tracking-tight text-[#1C1917] dark:text-[#F0F4F8] leading-[1.15] mb-6 text-center w-full"
      >
        <div className="text-[#1C1917] dark:text-[#F0F4F8] block min-h-[1.25em] mb-1 font-serif">
          <span>{activeTopic ? formattedActiveTopic : currentText}</span>
          {!activeTopic && (
            <span className="inline-block w-1 sm:w-1.5 h-[0.8em] bg-[#1C1917] dark:bg-[#F0F4F8] ml-1.5 animate-pulse align-middle"></span>
          )}
        </div>

        <div className="not-italic font-sans font-bold text-3xl sm:text-5xl md:text-6xl uppercase tracking-tight block text-[#1C1917] dark:text-[#F0F4F8]">
      ...
      <div className="h-14 sm:h-12 flex items-center justify-center mb-10 w-full">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="text-base sm:text-lg text-[#44403C] dark:text-[#B8C5D6] max-w-2xl font-medium text-center leading-relaxed"
          >
            {activeSubtext.isSpecial ? (
              <span>
                That thing you nodded along to but{' '}
                <span className="text-[#1C1917] dark:text-[#F0F4F8] font-extrabold underline underline-offset-4 decoration-[#1C1917]/50 dark:decoration-[#F0F4F8]/50">
                  never understood
                </span>
                ? Type it here.
              </span>
      ...
      <div className="w-full max-w-3xl mx-auto">
        ...
        >
          <div 
            className={`relative flex items-center bg-[#E8E0D5]/70 dark:bg-[#23354D]/80 border-2 border-[#1C1917] dark:border-[#8EA8C3]/50 rounded-2xl p-2 transition-all duration-300 shadow-md ${
              isFocused ? 'ring-2 ring-[#1C1917] dark:ring-[#F0F4F8]' : ''
            }`}
          >
            <div className="pl-3 pr-2 text-[#1C1917] dark:text-[#8EA8C3]">
            ...
            <input
              ...
              className="w-full bg-transparent text-[#1C1917] dark:text-[#F0F4F8] placeholder-[#44403C]/70 dark:placeholder-[#8EA8C3]/60 text-sm sm:text-base font-sans font-medium focus:outline-none py-2 px-1"
              disabled={isLoading}
            />
            ...
            <motion.button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl font-mono font-bold text-xs uppercase tracking-wider text-[#FAF6EE] dark:text-[#02122F] bg-[#1C1917] dark:bg-[#F0F4F8] hover:bg-[#2A2421] dark:hover:bg-[#E2E8F0] transition-all duration-200 shadow-md ${
                !topic.trim() || isLoading ? 'opacity-60 cursor-not-allowed' : ''
              }`}
            >
              {isLoading ? (
                <div className="flex items-center gap-2">
                  <div className="w-3.5 h-3.5 border-2 border-white dark:border-[#02122F] border-t-transparent rounded-full animate-spin"></div>
                  <span>WAIT</span>
                </div>
              ) : (
                <>
